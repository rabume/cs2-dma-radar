...
import de_train_radar from '/src/map/de_train_radar.png'
import cs_italy_radar from '/src/map/cs_italy_radar.png'
    ...
    methods: {
        ...
        updatePlayerMarker(data, knowMap) {
            ...
            data.forEach((item) => {
                if (item.alive) {
                    if (
                        (item.enemy && !this.showEnemies) ||
                        (!item.enemy && !this.showTeammates[item.compTeammateColor] && item.compTeammateColor !== -1)
                    ) {
                        return
                    }
